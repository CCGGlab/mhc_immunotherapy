...
studies<- c("Liu_2019", "Hugo_2016", "Gide_2019","Rizvi_2015", "Snyder_2014","VanAllen_2015") 
for(s in studies){
  ...
  # chisq.test(resp_t_all["highTMB",,]) # p-value = NA (Gide); 0.013 (Rizvi)
  chisq.test(resp_t_all["strongMHC1",,]) # p-value = 0.77; 0.72
  chisq.test(resp_t_all["strongMHC2",,]) # p-value = 0.0076; 0.23
  chisq.test(resp_t_all["strongMHCnorm",,]) # p-value = 0.027; 0.14
...
HR_df_all$treatment<- rep(c(rep("anti-PD1",4), rep("anti-CTLA4",2)),each=4)
...
fp <- ggplot(data=HR_df_all, aes(x=study, y=HR, ymin=ci_l, ymax=ci_h, label=paste("P =",signif(p,2)))) +
